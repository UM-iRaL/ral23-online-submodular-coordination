...
HORIZON = 100   # seconds
N_STEPS = 5000  # rounds of actions
N_TRIAL = 50    # number of Monte-Carlo simulations
BAR     = 5     # bar of targets' truncated input noise  
N_STEPS_TARGET = 5000 # rounds of random actions of targets
...
def OSG():
    ...
    for idx in range(N_TRIAL):
        ...
        # first generate random target trajectories
        for t in range(N_STEPS_TARGET):
            for target in targets:
                # first get targets' complete trajectories
                target.random_update_state(t, HORIZON, BAR)
        ...
        for t in range(N_STEPS):
            ...
            for target in targets:
                # assign targets with corresponding states at that time step
                # to allow for computing losses later
                target.state = target.traj[int((t+1) * N_STEPS_TARGET / N_STEPS)]
    ...
    # all_min_dist = np.load('all_'+str(HORIZON)+'_'+str(N_STEPS)+'.npy')
    ...
    # # plt.savefig("random_line_"+str(HORIZON)+"s_"+str(N_STEPS)+"step_"+str(BAR)+"bar", bbox_inches='tight')
    # # box + random
    # plt.xlim(-35, 35)
    # plt.ylim(-10, 35) 
    # plt.xlabel("X", fontname="Times New Roman", fontsize=20)
    # plt.ylabel("Y", fontname="Times New Roman", fontsize=20)
    # plt.savefig("random_box_"+str(HORIZON)+"s_"+str(N_STEPS)+"step_"+str(BAR)+"bar", bbox_inches='tight')
    # plt.show()
    ...
    # plt.legend(prop={'family':'Times New Roman', 'size':20}, loc='upper left')#, frameon=False)
    plt.xticks(fontname="Times New Roman", fontsize=20)
    plt.yticks(fontname="Times New Roman", fontsize=20)
    ...
    # plt.savefig("dist_"+str(N_TRIAL)+"trial_random_line_"+str(HORIZON)+"s_"+str(N_STEPS)+"step_"+str(BAR)+"bar", bbox_inches='tight')
    # box + random
    plt.ylim(0, 31) 
    plt.gca().set_aspect(aspect=1.2)
    plt.savefig("dist_"+str(N_TRIAL)+"trial_random_box_"+str(HORIZON)+"s_"+str(N_STEPS)+"step_"+str(BAR)+"bar", bbox_inches='tight')
    plt.show()
...
def create_target(states):
    targets = []
    for i in range(len(states)):
        targets.append(Target(states[i], time_step=HORIZON/N_STEPS_TARGET, idx=i, color=[1,1,1]))
    return targets
